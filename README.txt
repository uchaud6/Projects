...
Hello! My project four is about creating functions that help to analyze a dataset about video game sales. The dataset is analyzed in csv format, and the original dataset can be found at
"https://www.kaggle.com/sidtwr/videogames-sales-dataset?select=Video_Games_Sales_as_at_22_Dec_2016.csv". The csv file has relavent headers of 'Name, Platform, Year_of_Release, Genre, 
Global_Sales, Developer, Rating' (headers utilized in programming). Using the csv module's csv.DictReader() function to the process the data, I plan to write a program that covers five 
functions. 

The highestsales(year) function is designed to find the highest selling game of a given year based on the given argument to the function. The highestgenres() function is designed to order 
all the genres of the games in the dataset from highest to lowest based off of how many games have been sold to each respective genre. The highestplatforms() function is designed to order 
all the consoles/platforms of the games in the dataset from highest to lowest based off of how many games have been sold with each console. The highestESRB() function is designed to 
order all the ESRB ratings from highest to lowest based off how many games have been sold with each respective rating. The highestdevelopers() function is designed to order the ten 
developer companies that have helped to sell the most games by order of highest to lowest.


Files Given:

project4.py - Main program file with five functions for data analysis

videogames.csv - Video game data file that is analyzed by project4.py

test_data.py - File used for testing project4.py


How to Test/Usability:

The file test_data.py will use project4.py as a module and call each one of its functions. test_data.py will declare when calling a specific function and give lines of spacing between 
function outputs for readability and inspection of desired output.

The files given may not be renamed in order for all program files to run properly without changes. The files should all be saved in the same directory and when running the file, the
python interperter must also be in the corresponding directory.
...
Data Findings/Function Outputs:
...
not available games have 2.42 million games sold

	highestplatforms() produced:

The PS2 console has sold 1255.6399999999871 million games 1. play
The X360 console has sold 971.6299999999992 million games 2. micrro
The PS3 console has sold 939.4299999999982 million games 3. play
The Wii console has sold 908.129999999998 million games 4. nint
The DS console has sold 807.0999999999868 million games 5. nin
The PS console has sold 730.679999999997 million games 6. play
The GBA console has sold 318.499999999998 million games 7. nin
The PS4 console has sold 314.22999999999905 million games 8. play
The PSP console has sold 294.2999999999953 million games 9. play
The PC console has sold 260.29999999999643 million games 10. PC
...
2851.210000000242 million games with the ESRB rating of a not given rating have been sold.
...
Not given developing companys have helped to sell 2829.46000000023 million game copies
